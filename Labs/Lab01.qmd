...
Select the appropriate download for your computer's operating system.
...
:::{.aside}
For Boston University students, if you encounter difficulties or if your primary device is an tablet, you may alternatively  access a full installation of R/RStudio through the [_BU Common_](https://www.bu.edu/tech/services/cccs/desktop/computer-labs/common/access/#webbrowser) Windows virtual desktop.
::: 
...
On the Posit website, locate the appropriate package for your operating system.

![](/assets/lab01-download-rstudio.png)

On an **Apple Mac/MacBook**, you should download and open the `.DMG` file. You will see the RStudio app and your Applications folder. **Drag the RStudio app into your applications folder**. 

On a **Windows PC**, you should download and run the Windows executable installer.
...
While R is the programming language, RStudio is a graphical interface that provides us with tools to edit R code, view plots and help pages, debug scripts, and more.

![At a high level, RStudio's graphical interface provides access to the R Console, a built in text-editor (for writing scripts), and functionality for viewing and exporting plots. <br>[Image credited to the authors of [R4DS](https://r4ds.hadley.nz/workflow-scripts.html), reproduced here under license [CC BY-NC-ND 3.0](https://creativecommons.org/licenses/by-nc-nd/3.0/us/).]{.small}](https://r4ds.hadley.nz/diagrams/rstudio/script.png)
...
A lot of output will appear in the console as the package is installing. When it finishes, the package has been installed. We won't need to run this again; so there's no need to keep this line in your script.

## Demonstrating Graphics in R

As a motivating example, let's consider the following short but complete script which will plot for us. We'll work through this code line-by-line in class to understand what it does.

The script will make a plot from a dataset built-in to R called `ChickWeight`. The data comes from **an experiment with four conditions**: diets assigned to hatchling chicks. Their weight in grams is recorded every two days in the built in **dataframe** (table).

Type the following R code into a new script in your RStudio **Editor** pane. Click Save and save the script on your computer (e.g. `Documents/PS211/lab1.R`). Then click **Run**.
...
:::{.aside}
In this first example, we're looking at exp
:::
